...
export type SessionParticipantProps = z.infer<
  typeof SessionParticipantPropsSchema
>;
...
export class Session {
  ...
  addParticipant(
    userId: UserIdVO,
    role: SessionRole,
    options?: { teamId?: string | null; joinedAt?: Date },
  ): void {
    const exists = this.props.participants.some(
      (p) => p.userId.value === userId.value && !p.leftAt,
    );
    if (exists) {
